...
combinedplots_rawsupp <- ggarrange(vatt, vimphyp, vbeh, watt, wimphyp, wbeh, 
                                     labels = c("A", "B", "C", "D", "E", "F"),
                                     ncol = 3, nrow = 2)
print(combinedplots_rawtot)
print(combinedplots_rawsupp)
...
modelCplot <- ggplot(bootstrap_results_multi3, 
                     aes(x = RateRatio, y = Outcome, color = DietPattern)) +
  geom_point(size = 3, position = position_dodge(width = 0.3)) +  # Plot estimates
  geom_errorbarh(aes(xmin = Conf.Low, xmax = Conf.High), height = 0.2, position = position_dodge(width = 0.3)) +  # CI bars
  geom_text(aes(label = case_when(
...
combinedplots <- ggarrange(modelAplot,modelBplot, modelCplot, labels = c("A", "B", "C"),
                           ncol = 3, nrow = 1, common.legend = TRUE, legend = "bottom", widths = c(1.15, 1, 1))
...
ggplot(FGspeardietpatterns_adhdrs, aes(x = x_dodged, y = estimate, color = diet)) +
  ...
  scale_color_manual(values = c("Diet_1FG" = "deeppink2", "Diet_2FG" = "darkturquoise"),
                     ...
                     name = "Diet Pattern") +
  scale_x_continuous(breaks = c(1, 2, 3, 4), labels = c("Total ADHD-RS Score", 
                                                        "Impulsivity/Hyperactivity", 
                                                        "Oppositional Behavior", 
                                                        "Inattention")) +
  scale_y_continuous(limits = c(-0.25, 0.25)) +
  labs(title = "Spearman Correlation with ADHD Measures by Diet Pattern",
       x = "ADHD Measure",
