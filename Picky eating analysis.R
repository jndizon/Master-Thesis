...
ggplot(spear_picky, aes(x = exposure_1, y = estimate)) +
  ...
  geom_point(size = 5, color = "darkgoldenrod2") +
  ...
  scale_x_discrete(labels = c("total_score" = "Total ADHD-RS\nScore", 
                              "implus_hyperact" = "Impulsivity/\nHyperactivity", 
                              "behavior_subscale" = "Oppositional\nBehavior", 
                              "socialcircumstances" = "Social\nCircumstances", 
                              "attention" = "Inattention",
                              "Diet_1FG" = "Varied Diet\nPattern",
                              "Diet_2FG" = "Western Diet\nPattern")) + # Renaming y-axis labels
  labs(y = "Spearman Correlation Estimate", x = "") + 
...
# bray-curtis (already done in Phyloseq pre-process + diversity)
